...
export function calculateLateFee(booking, orgSettings) {
    ...
    const now = new Date();
    const returnDayStart = new Date(booking.end_date + "T00:00:00");
    const graceMs = (orgSettings.late_fee_grace_hours ?? 2) * 60 * 60 * 1000;
    const deadline = new Date(addDays(returnDayStart, 1).getTime() + graceMs);

    if (now <= deadline) {
        return { isLate: false, fee: 0, daysLate: 0, hoursLate: 0 };
    }

    // Charge only for time beyond the allowed return deadline.
    const msLate = now - deadline;
