...
/**
 * CalendarView — M7 Enhanced Availability Calendar
 * Four modes: day, week, month, gantt (legacy)
 */
export default function CalendarView() {
    ...
    const [fullModal, setFullModal] = useState(null); // { booking? } for full BookingModal
    ...
    const filteredBookings = useMemo(() => {
        ...
        return bookings.bookings.filter(b => {
            ...
            for (const sf of statusFilter) {
                ...
                if (sf === "maintenance") return false; // handled separately
            }
            return false;
        });
    ...
    // Handlers
    const handleClickSlot = useCallback((date, bikeId, hour) => {
        setQuickBooking({ date, bikeId, hour });
    }, []);
    ...
    const handleQuickSave = useCallback(async (bookingData) => {
        try {
            let customerId = null;
            if (bookingData.customer_email || bookingData._first_name) {
                const { data: newCustomer, error: customerError } = await customers.create({
                    first_name: bookingData._first_name || bookingData.customer_name?.split(" ")[0] || "Gast",
                    last_name: bookingData._last_name || bookingData.customer_name?.split(" ").slice(1).join(" ") || "Gast",
